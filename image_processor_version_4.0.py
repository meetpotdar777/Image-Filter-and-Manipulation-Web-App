...
# Global variables to hold the current original and processed image data (raw NumPy arrays)
# These will be updated on image upload and processing.
# Using locks for thread-safe access to these global resources.
current_original_img_np = None
current_processed_img_np = None
...
FILTERS = {
    ...
    "resize": "Resize Image",
    "color_adjust": "Color Channel Adjust",
    "watermark_text": "Text Watermark",
    "crop": "Crop Image (Interactive)", # New filter
    "histogram_equalization": "Histogram Equalization", # New filter
    "vignette": "Vignette Effect", # New filter
    "cartoonify": "Cartoonify" # New filter
}
...
def apply_filter(image_np, filter_name, brightness=0, contrast=1, sharpen_amount=0,
    ...
    if filter_name == "grayscale":
        ...
        # Create a radial mask, ensuring it's float32 for multiplication
        center_x, center_y = cols // 2, rows // 2
        Y, X = np.ogrid[:rows, :cols]
        dist_from_center = np.sqrt((X - center_x)**2 + (Y - center_y)**2)
        
        # Normalize distance to be from 0 (center) to 1 (max_radius)
        max_radius = np.sqrt(center_x**2 + center_y**2)
        normalized_dist = dist_from_center / max_radius

        # Create a smooth circular gradient mask
        # Intensity controls how sharp or gradual the vignette is
        circular_mask = (1 - normalized_dist).clip(0, 1) # Linear decay from center to edge
        
        # Apply power for falloff based on vignette_intensity (higher intensity = sharper edge)
        # We use (1 / max(0.1, vignette_intensity)) to invert the slider's effect
        circular_mask = circular_mask ** (1.0 / max(0.1, vignette_intensity * 2)) # Adjust power
        
        # Boost center brightness slightly to prevent it from getting too dark
        circular_mask = circular_mask * (1.0 + vignette_intensity * 0.5) 
        circular_mask = np.clip(circular_mask, 0, 1).astype(np.float32) # Ensure it's float32

        # Convert to 3 channels for element-wise multiplication with the image
        mask_3_channel = np.stack([circular_mask, circular_mask, circular_mask], axis=-1)
        
        # Apply mask to image (darken outside, keep center bright)
        # Ensure both operands for cv2.multiply are float32
    ...
    if watermark_text and (filter_name == "watermark_text" or (filter_name != "original" and watermark_text)): # Apply watermark if filter is chosen or if text is present with another filter
...
<body class="bg-gradient-to-br from-indigo-100 to-purple-100 flex items-center justify-center min-h-screen p-4 sm:p-6 md:p-8">
    ...
    <script>
        ...
        function renderDynamicControls(filterName) {
            ...
            if (filterName === 'brightness_contrast') {
                ...
                document.getElementById('vignetteIntensitySlider').addEventListener('input', () => {
                    document.getElementById('vignetteIntensityValue').textContent = document.getElementById('vignetteIntensitySlider').value;
                    triggerImageProcessing();
                });
            }
        }
        ...
        function triggerImageProcessing() {
            ...
            // histogram_equalization and cartoonify do not require extra parameters.
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            // The originalImageContainer HTML is now stable, no need to re-inject.
...
def process_image_route():
    ...
    crop_coords = data.get('crop_coords', None) # New parameter for cropping
    vignette_intensity = float(data.get('vignette_intensity', 0.5)) # New parameter for vignette
