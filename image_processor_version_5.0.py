...
def apply_filter(image_np, filter_name, brightness=0, contrast=1, sharpen_amount=0,
    ...
    if filter_name == "grayscale":
    ...
    elif filter_name == "adaptive_threshold": # New Adaptive Threshold
        gray_image = cv2.cvtColor(processed_image, cv2.COLOR_BGR2GRAY)
    ...
    elif filter_name == "hsl_adjust": # New HSL Adjustment
        hls_image = cv2.cvtColor(processed_image, cv2.COLOR_BGR2HLS)
        h, l, s = cv2.split(hls_image)

        # Hue: 0-179 in OpenCV, adjust by angle
        h = np.clip(h.astype(np.int32) + hue_adjust // 2, 0, 179).astype(np.uint8) # Hue is 0-179 in OpenCV
        # Lightness/Saturation: 0-255
    ...
    elif filter_name == "pencil_sketch": # New Pencil Sketch Filter
        if len(processed_image.shape) == 3 and processed_image.shape[2] == 3:
            # Convert to grayscale
            gray_image = cv2.cvtColor(processed_image, cv2.COLOR_BGR2GRAY)
        else:
            gray_image = processed_image # Already grayscale

        # Invert the grayscale image
        inverted_image = cv2.bitwise_not(gray_image)

        # Apply Gaussian blur to the inverted image
        blurred_image = cv2.GaussianBlur(inverted_image, (21, 21), 0) # Larger kernel for smoother sketch

        # Invert the blurred image
        inverted_blurred_image = cv2.bitwise_not(blurred_image)

        # Blend the grayscale image with the inverted blurred image using a "dodge" blend mode
        # This is approximated by dividing the original image by the inverted blurred image
        # Adding a small constant (1) to avoid division by zero
        ...
        # Convert back to 3 channels for consistent display if it was originally grayscale
    ...
    if filter_name == "image_overlay" and overlay_image_data:
        try:
            ...
            overlay_img_np = cv2.imdecode(nparr_overlay, cv2.IMREAD_UNCHANGED) # IMREAD_UNCHANGED to read alpha channel

            if overlay_img_np is None:
                print("WARNING: Could not decode overlay image.")
                # Continue without overlay if decoding fails
            else:
                # Resize overlay image to match the processed_image dimensions
                ...
                if overlay_resized.shape[2] == 4: # If overlay has an alpha channel
                    ...
                    # Ensure main image is float32 for calculations
                ...
                else: # No alpha channel, simple weighted blend
    ...
    # If "original" or an unknown filter, return original image (no changes unless resize applied earlier)
    elif filter_name == "original" and resize_percent == 100: # Removed check for resize_percent, it's handled implicitly now
        return image_np
...
<body class="bg-gradient-to-br from-indigo-100 to-purple-100 flex items-center justify-center min-h-screen p-4 sm:p-6 md:p-8">
    ...
    <script>
        ...
        function renderDynamicControls(filterName) {
            ...
            if (filterName === 'brightness_contrast') {
            ...
            } else if (filterName === 'adaptive_threshold') { // New Adaptive Threshold Controls
                dynamicControls.classList.remove('hidden');
                dynamicControls.innerHTML = `
                ...
                document.getElementById('adaptiveBlockSizeSlider').addEventListener('input', () => {
                    ...
                    if (val % 2 === 0) val += 1; // Ensure it's odd
                    document.getElementById('adaptiveBlockSizeValue').textContent = val;
                    document.getElementById('adaptiveBlockSizeSlider').value = val; // Update slider to reflect odd value
                    triggerImageProcessing();
                });
            ...
            } else if (filterName === 'hsl_adjust') { // New HSL Controls
                dynamicControls.classList.remove('hidden');
                dynamicControls.innerHTML = `
            ...
            } else if (filterName === 'blur') { // Adjustable Gaussian Blur Controls
                dynamicControls.classList.remove('hidden');
                dynamicControls.innerHTML = `
                ...
                document.getElementById('gaussianKernelSlider').addEventListener('input', () => {
                    ...
                    document.getElementById('gaussianKernelSlider').value = val; // Update slider to reflect odd value
                    triggerImageProcessing();
                });
            }
            ...
            } else if (filterName === 'image_overlay') { // New Image Overlay Controls
                dynamicControls.classList.remove('hidden');
                dynamicControls.innerHTML = `
                ...
                cropControls.classList.remove('hidden'); // Show crop controls
                // Deactivate drawing mode if active from previous session
                deactivateCropDrawing();
                performCropBtn.classList.add('hidden'); // Hide crop button until selection
        ...
        function activateCropDrawing() {
            ...
            cropSelectionRect.classList.add('hidden'); // Hide previous rect
            performCropBtn.classList.add('hidden'); // Hide crop button
            isDrawing = false; // Reset drawing state
            cropRect = { x: 0, y: 0, width: 0, height: 0 }; // Reset crop coordinates
        ...
        function startDrawing(e) {
            isDrawing = true;
            const rect = originalImagePreview.getBoundingClientRect(); // Use image's actual displayed size
            ...
            performCropBtn.classList.add('hidden'); // Hide crop button until valid selection
        }
        ...
        function drawRect(e) {
            ...
            cropSelectionRect.style.left = `${Math.min(startX, currentX)}px`; // Adjust start point for drawing backwards
            ...
            // Update cropRect (relative to image's natural size)
            // Calculate scale based on *displayed image size* versus *natural image size*
            ...
            // Use Math.min/max for robust calculation when dragging backwards
            ...
            // Ensure cropRect values are non-negative and within bounds of the natural image
        ...
        function stopDrawing() {
            isDrawing = false;
            // No need to hide the rect, it stays visible for confirmation
        }

        startCroppingBtn.addEventListener('click', () => {
            activateCropDrawing();
            // Important: attach listeners to cropOverlay, not originalImageContainer itself
        ...
        performCropBtn.addEventListener('click', () => {
            if (cropRect.width > 0 && cropRect.height > 0) {
                triggerImageProcessing();
                deactivateCropDrawing(); // Deactivate drawing mode after initiating crop
                cropSelectionRect.classList.add('hidden'); // Hide selection rectangle
            } else {
        ...
        async function processImage(filterName, params = {}) {
            ...
            // If filter is 'crop', send the current cropRect
            if (filterName === 'crop') {
                if (cropRect.width === 0 || cropRect.height === 0) {
            ...
            // Include overlay image data if available and the filter is image_overlay
        ...
        imageUpload.addEventListener('change', function(event) {
            ...
            if (file) {
                ...
                reader.onload = function(e) {
                    ...
                    originalImagePreview.classList.remove('hidden'); // Show image preview
                    originalImagePlaceholder.classList.add('hidden'); // Hide placeholder text
                    ...
                    originalImagePreview.onload = () => {
                        ...
                        containerClientWidth = originalImageContainer.clientWidth; // This will be the max width of the container
                        containerClientHeight = originalImageContainer.clientHeight; // This will be the max height of the container
                    ...
                    deactivateCropDrawing(); // Ensure crop mode is off
                    cropSelectionRect.classList.add('hidden'); // Hide rect
                    performCropBtn.classList.add('hidden'); // Hide crop button
                ...
                currentOverlayImageBase64 = null; // Clear overlay image on main image reset
                currentProcessedImageBase64 = null;
                originalImagePreview.classList.add('hidden'); // Hide image preview
                originalImagePlaceholder.classList.remove('hidden'); // Show placeholder text
                processedImageContainer.innerHTML = `<p class="text-gray-500">Processed image will appear here.</p>`;
                filterSelect.value = "original";
        ...
        filterSelect.addEventListener('change', function() {
            ...
            deactivateCropDrawing(); // Always deactivate cropping when filter changes
            cropSelectionRect.classList.add('hidden'); // Hide selection rectangle
            performCropBtn.classList.add('hidden'); // Hide crop button
            if (currentImageBase64) {
        ...
        resetBtn.addEventListener('click', () => {
            ...
            currentImageBase64 = null;
            currentOverlayImageBase64 = null; // Clear overlay on full reset
            currentProcessedImageBase64 = null;
            originalImagePreview.src = "";
            originalImagePreview.classList.add('hidden'); // Hide image
            originalImagePlaceholder.classList.remove('hidden'); // Show placeholder
            processedImageContainer.innerHTML = `<p class="text-gray-500">Processed image will appear here.</p>`;
            filterSelect.value = "original";
            renderDynamicControls("original"); // Reset dynamic controls
            downloadBtn.disabled = true;
        ...
        function triggerImageProcessing() {
            ...
            if (selectedFilter === 'brightness_contrast') {
            ...
            } else if (selectedFilter === 'blur') { // Gaussian Blur with adjustable kernel
                ...
                // currentOverlayImageBase64 is passed directly to processImage in the payload construction
            ...
            // For 'crop', the crop_coords are handled separately in processImage based on the current cropRect state.
            // histogram_equalization, cartoonify, and pencil_sketch do not require extra parameters from sliders.
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            // Just ensure initial state is correct for image preview and placeholder.
            originalImagePreview.classList.add('hidden'); // Hide image if not already
            originalImagePlaceholder.classList.remove('hidden'); // Show placeholder text
        });
    </script>
</body>
...
def process_image_route():
    ...
    overlay_image_data = data.get('overlay_image_data', None) # New: for image overlay
    overlay_opacity = float(data.get('overlay_opacity', 0.5)) # New: for image overlay opacity
    ...
    try:
        ...
        # Store the original image in the global variable for future operations
        with image_data_lock:
            global current_original_img_np
            current_original_img_np = img_np.copy()

        # Logic for processing:
        # If filter is 'original', we use the uploaded image directly.
        # Otherwise, process the original uploaded image with the chosen filter.
