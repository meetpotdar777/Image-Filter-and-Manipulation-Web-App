...
if not os.path.exists(UPLOAD_FOLDER):
    # CORRECTED TYPO: Changed UPLOADED_FOLDER to UPLOAD_FOLDER
...
FILTERS = {
    ...
    "hsl_adjust": "HSL Color Adjust",
    "color_temperature": "Color Temperature",
    "exposure_gamma": "Exposure/Gamma Correction",
    "dithering": "Dithering (Monochrome)",
    "selective_color": "Selective Color Adjustment",
    "color_splash": "Color Splash Effect",
    "warm_filter": "Photo Filter: Warm",
    "cool_filter": "Photo Filter: Cool",
    "vintage_filter": "Photo Filter: Vintage",
    "bw_red_tint": "Photo Filter: B&W Red Tint",
    "watermark_text": "Text Watermark",
    "image_overlay": "Image Overlay / Blend",
...
def apply_filter(image_np, filter_name, brightness=0, contrast=1, sharpen_amount=0,
                 ...
                 selective_color_target="red", selective_hue_adjust=0, selective_saturation_adjust=0, selective_lightness_adjust=0,
                 color_splash_target_color="#FF0000", color_splash_tolerance=30,
    ...
    Args:
        ...
        selective_color_target (str): Target color for selective color adjustment.
        selective_hue_adjust (int): Hue adjustment for selective color.
        selective_saturation_adjust (int): Saturation adjustment for selective color.
        selective_lightness_adjust (int): Lightness adjustment for selective color.
        color_splash_target_color (str): Hex color to keep in Color Splash.
        color_splash_tolerance (int): Tolerance for hue matching in Color Splash.
    ...
    if filter_name == "grayscale":
    ...
    elif filter_name == "color_temperature":
    ...
    elif filter_name == "exposure_gamma":
        if exposure_val <= 0: exposure_val = 0.01
        gamma = 1.0 / exposure_val
        
        table = np.array([((i / 255.0) ** gamma) * 255 for i in np.arange(0, 256)]).astype("uint8")
        processed_image = cv2.LUT(processed_image, table)
    elif filter_name == "dithering":
        if len(processed_image.shape) == 3 and processed_image.shape[2] == 3:
            gray_image = cv2.cvtColor(processed_image, cv2.COLOR_BGR2GRAY)
        else:
    ...
    elif filter_name == "selective_color":
        if len(processed_image.shape) == 3 and processed_image.shape[2] == 3:
            ...
            color_ranges = {
                ...
                "green": ([35, 120, 70], [80, 255, 255]),
                "blue": ([100, 120, 70], [140, 255, 255]),
                "yellow": ([20, 120, 70], [35, 255, 255]),
                "magenta": ([140, 120, 70], [170, 255, 255]),
                "cyan": ([80, 120, 70], [100, 255, 255]),
            }

            if selective_color_target in color_ranges:
                lower1, upper1 = color_ranges[selective_color_target][0], color_ranges[selective_color_target][1]
                mask1 = cv2.inRange(hsv_image, np.array(lower1), np.array(upper1))
                
                mask = mask1
                if selective_color_target == "red" and len(color_ranges[selective_color_target]) > 2:
                    lower2, upper2 = color_ranges[selective_color_target][2], color_ranges[selective_color_target][3]
                    mask2 = cv2.inRange(hsv_image, np.array(lower2), np.array(upper2))
                    mask = cv2.bitwise_or(mask1, mask2)

                adjusted_hsv = hsv_image.copy()

                hue_adjusted_portion = np.clip(h.astype(np.int32) + selective_hue_adjust // 2, 0, 179).astype(np.uint8)
                sat_adjusted_portion = np.clip(s.astype(np.int32) + (selective_saturation_adjust * 255 / 100), 0, 255).astype(np.uint8)
                val_adjusted_portion = np.clip(v.astype(np.int32) + (selective_lightness_adjust * 255 / 100), 0, 255).astype(np.uint8)

                adjusted_hsv[:, :, 0] = np.where(mask > 0, hue_adjusted_portion, h)
                adjusted_hsv[:, :, 1] = np.where(mask > 0, sat_adjusted_portion, s)
                adjusted_hsv[:, :, 2] = np.where(mask > 0, val_adjusted_portion, v)
                
                processed_image = cv2.cvtColor(adjusted_hsv, cv2.COLOR_HSV2BGR)
            else:
                print(f"Warning: Selective color target '{selective_color_target}' not recognized.")
                processed_image = image_np.copy()
        else:
            print("Warning: Selective color filter requires a color image.")
            processed_image = image_np.copy()

    elif filter_name == "color_splash":
        if len(processed_image.shape) == 3 and processed_image.shape[2] == 3:
            hsv_image = cv2.cvtColor(processed_image, cv2.COLOR_BGR2HSV)
            
            hex_color = color_splash_target_color.lstrip('#')
            rgb_target = tuple(int(hex_color[i:i+2], 16) for i in (0, 2, 4))
            bgr_target = np.array([[rgb_target[::-1]]], dtype=np.uint8)
            hsv_target = cv2.cvtColor(bgr_target, cv2.COLOR_BGR2HSV)[0][0]

            target_hue = hsv_target[0]
            
            lower_hue = (target_hue - color_splash_tolerance) % 180
            upper_hue = (target_hue + color_splash_tolerance) % 180

            if lower_hue < upper_hue:
                mask = cv2.inRange(hsv_image, np.array([lower_hue, 50, 50]), np.array([upper_hue, 255, 255]))
            else:
                mask1 = cv2.inRange(hsv_image, np.array([lower_hue, 50, 50]), np.array([179, 255, 255]))
                mask2 = cv2.inRange(hsv_image, np.array([0, 50, 50]), np.array([upper_hue, 255, 255]))
                mask = cv2.bitwise_or(mask1, mask2)
            
            gray_image_3_channel = cv2.cvtColor(cv2.cvtColor(processed_image, cv2.COLOR_BGR2GRAY), cv2.COLOR_GRAY2BGR)

            processed_image = np.where(mask[:,:,np.newaxis] > 0, processed_image, gray_image_3_channel)
        else:
            print("Warning: Color Splash filter requires a color image.")
            processed_image = image_np.copy()

    elif filter_name == "warm_filter":
        processed_image = cv2.addWeighted(processed_image, 1.0, np.zeros(processed_image.shape, processed_image.dtype), 0, 0)
        b, g, r = cv2.split(processed_image)
        r = np.clip(r.astype(np.int32) + 50, 0, 255).astype(np.uint8)
        g = np.clip(g.astype(np.int32) + 20, 0, 255).astype(np.uint8)
        processed_image = cv2.merge([b, g, r])
        processed_image = processed_image.astype(np.float32) * 1.1
        processed_image = np.clip(processed_image, 0, 255).astype(np.uint8)
    elif filter_name == "cool_filter":
        processed_image = cv2.addWeighted(processed_image, 1.0, np.zeros(processed_image.shape, processed_image.dtype), 0, 0)
        b, g, r = cv2.split(processed_image)
        b = np.clip(b.astype(np.int32) + 50, 0, 255).astype(np.uint8)
        g = np.clip(g.astype(np.int32) + 20, 0, 255).astype(np.uint8)
        processed_image = cv2.merge([b, g, r])
        processed_image = processed_image.astype(np.float32) * 1.1
        processed_image = np.clip(processed_image, 0, 255).astype(np.uint8)
    elif filter_name == "vintage_filter":
        kernel_sepia = np.array([[0.272, 0.534, 0.131],
                                 [0.349, 0.686, 0.168],
                                 [0.393, 0.769, 0.189]])
        processed_image = cv2.transform(processed_image, kernel_sepia.T)
        processed_image = np.clip(processed_image, 0, 255).astype(np.uint8)

        processed_image = cv2.cvtColor(processed_image, cv2.COLOR_BGR2HSV)
        h, s, v = cv2.split(processed_image)
        ...
        processed_image = processed_image.astype(np.float32) * 0.9 + 20
        processed_image = np.clip(processed_image, 0, 255).astype(np.uint8)
        processed_image = cv2.GaussianBlur(processed_image, (3, 3), 0)
    elif filter_name == "bw_red_tint":
        if len(processed_image.shape) == 3 and processed_image.shape[2] == 3:
            gray_image = cv2.cvtColor(processed_image, cv2.COLOR_BGR2GRAY)
            gray_image_3_channel = cv2.cvtColor(gray_image, cv2.COLOR_GRAY2BGR)
            
            b, g, r = cv2.split(gray_image_3_channel)
            r = np.clip(r.astype(np.int32) + 70, 0, 255).astype(np.uint8)
            b = np.clip(b.astype(np.int32) - 30, 0, 255).astype(np.uint8)
            g = np.clip(g.astype(np.int32) - 10, 0, 255).astype(np.uint8)
            processed_image = cv2.merge([b, g, r])
        else:
            processed_image = cv2.cvtColor(processed_image, cv2.COLOR_GRAY2BGR)
            b, g, r = cv2.split(processed_image)
...
<head>
    ...
    <style>
        ...
        .crop-selection-rect {
            ...
            cursor: move;
            /* resize: both; */ /* Removed resize for now to simplify initial debugging, re-add later if needed */
...
<body class="bg-gradient-to-br from-indigo-100 to-purple-100 flex items-center justify-center min-h-screen p-4 sm:p-6 md:p-8">
    <div class="bg-white p-6 md:p-8 rounded-xl shadow-2xl max-w-2xl w-full text-center border-t-8 border-indigo-600 transform hover:scale-105 transition-transform duration-300">
        ...
        <div class="flex flex-col sm:flex-row justify-center gap-4 mb-6">
            ...
            <select id="filterSelect" class="form-select px-4 py-2 rounded-full shadow-sm text-gray-700 bg-white border border-gray-300 focus:outline-none focus:ring-2 focus:ring-indigo-500">
                ...
                <option value="selective_color">Selective Color Adjustment</option>
                <option value="color_splash">Color Splash Effect</option>
                <option value="warm_filter">Photo Filter: Warm</option>
                <option value="cool_filter">Photo Filter: Cool</option>
                <option value="vintage_filter">Photo Filter: Vintage</option>
                <option value="bw_red_tint">Photo Filter: B&W Red Tint</option>
                <option value="watermark_text">Text Watermark</option>
                <option value="image_overlay">Image Overlay / Blend</option>
                <option value="crop">Crop Image (Interactive)</option>
    ...
    <script>
        ...
        function renderDynamicControls(filterName) {
            console.log(`[Render Controls] Filter selected: ${filterName}`);
            ...
            // Hide the crop overlay and selection rect when changing filters
            ...
            } else if (filterName === 'selective_color') {
                dynamicControls.classList.remove('hidden');
                dynamicControls.innerHTML = `
                    <h3 class="text-lg font-medium text-gray-700">Selective Color Adjustment</h3>
                    <div class="w-full max-w-xs mb-2">
                        <label for="selectiveColorTarget" class="block text-sm font-medium text-gray-700">Target Color:</label>
                        <select id="selectiveColorTarget" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                    ...
                    <div class="w-full max-w-xs">
                        <label for="selectiveHueAdjustSlider" class="block text-sm font-medium text-gray-700">Hue: <span id="selectiveHueAdjustValue">0</span></label>
                        <input type="range" id="selectiveHueAdjustSlider" min="-180" max="180" value="0" class="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer range-lg">
                    </div>
                    <div class="w-full max-w-xs">
                        <label for="selectiveSaturationAdjustSlider" class="block text-sm font-medium text-gray-700">Saturation: <span id="selectiveSaturationAdjustValue">0</span></label>
                        <input type="range" id="selectiveSaturationAdjustSlider" min="-100" max="100" value="0" class="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer range-lg">
                    </div>
                    <div class="w-full max-w-xs">
                        <label for="selectiveLightnessAdjustSlider" class="block text-sm font-medium text-gray-700">Lightness: <span id="selectiveLightnessAdjustValue">0</span></label>
                        <input type="range" id="selectiveLightnessAdjustSlider" min="-100" max="100" value="0" class="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer range-lg">
                    </div>
                    <p class="text-xs text-gray-500">Adjust HSL for specific color channels. Note: HSL ranges differ for HSV in OpenCV.</p>
                `;
                document.getElementById('selectiveColorTarget').addEventListener('change', triggerImageProcessing);
                document.getElementById('selectiveHueAdjustSlider').addEventListener('input', () => {
                    document.getElementById('selectiveHueAdjustValue').textContent = document.getElementById('selectiveHueAdjustSlider').value;
                    triggerImageProcessing();
                });
                document.getElementById('selectiveSaturationAdjustSlider').addEventListener('input', () => {
                    document.getElementById('selectiveSaturationAdjustValue').textContent = document.getElementById('selectiveSaturationAdjustSlider').value;
                    triggerImageProcessing();
                });
                document.getElementById('selectiveLightnessAdjustSlider').addEventListener('input', () => {
                    document.getElementById('selectiveLightnessAdjustValue').textContent = document.getElementById('selectiveLightnessAdjustSlider').value;
                    triggerImageProcessing();
                });
            } else if (filterName === 'color_splash') {
                dynamicControls.classList.remove('hidden');
                dynamicControls.innerHTML = `
                    <h3 class="text-lg font-medium text-gray-700">Color Splash Effect</h3>
                    <div class="w-full max-w-xs mb-2">
                        <label for="colorSplashTargetColor" class="block text-sm font-medium text-gray-700">Keep Color:</label>
                        <input type="color" id="colorSplashTargetColor" value="#FF0000" class="mt-1 w-full h-10 cursor-pointer">
                    </div>
                    <div class="w-full max-w-xs">
                        <label for="colorSplashToleranceSlider" class="block text-sm font-medium text-gray-700">Tolerance: <span id="colorSplashToleranceValue">30</span></label>
                        <input type="range" id="colorSplashToleranceSlider" min="0" max="100" value="30" class="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer range-lg">
                    </div>
                    <p class="text-xs text-gray-500">Select a color to keep, others will be grayscale. Adjust tolerance for matching.</p>
                `;
                document.getElementById('colorSplashTargetColor').addEventListener('input', triggerImageProcessing);
                document.getElementById('colorSplashToleranceSlider').addEventListener('input', () => {
                    document.getElementById('colorSplashToleranceValue').textContent = document.getElementById('colorSplashToleranceSlider').value;
                    triggerImageProcessing();
                });
        ...
        function activateCropDrawing() {
            console.log('[Cropping] activateCropDrawing called. Current image src:', originalImagePreview.src ? originalImagePreview.src.substring(0, 50) + '...' : 'None');
            if (!originalImagePreview.src) {
                showToast('Please upload an image first to crop!', 'error');
                console.log('[Cropping] No image uploaded, cannot activate cropping.');
                return;
            }
            cropOverlay.classList.remove('hidden'); // Ensure overlay is visible
            cropOverlay.classList.add('active'); // Add active class to apply pointer-events and background
            cropSelectionRect.classList.add('hidden'); // Keep rect hidden until drawing starts
            performCropBtn.classList.add('hidden');
            isDrawing = false;
            cropRect = { x: 0, y: 0, width: 0, height: 0 };
            showToast('Click and drag on the image to select the crop area. The image background will darken.', 'info');
            console.log('[Cropping] Crop overlay active. Ready to draw.');
            ...
            console.log('[Cropping] Mouse event listeners re-attached to cropOverlay.');
            
            // Add a temporary visual cue if needed
            cropOverlay.style.border = '2px solid red'; // Temporary border for debugging
            setTimeout(() => {
                cropOverlay.style.border = ''; // Remove after a short delay
            }, 500);
        }

        function deactivateCropDrawing() {
            console.log('[Cropping] deactivateCropDrawing called.');
            cropOverlay.classList.add('hidden'); // Hide overlay
            cropOverlay.classList.remove('active'); // Remove active class
            cropOverlay.removeEventListener('mousedown', startDrawing);
            cropOverlay.removeEventListener('mousemove', drawRect);
            cropOverlay.removeEventListener('mouseup', stopDrawing);
            isDrawing = false;
            console.log('[Cropping] Crop overlay deactivated and listeners removed.');
        }

        function startDrawing(e) {
            console.log('[Cropping] startDrawing called. Event:', e);
            isDrawing = true;
            // Get bounding rect of the original image *preview* for accurate coordinate mapping
            const rect = originalImagePreview.getBoundingClientRect(); 
            
            console.log(`[Cropping-START] originalImagePreview.getBoundingClientRect(): Left: ${rect.left}, Top: ${rect.top}, Width: ${rect.width}, Height: ${rect.height}`);

            // Calculate mouse position relative to the image itself
            startX = e.clientX - rect.left;
            startY = e.clientY - rect.top;
            
            // Clamp startX, startY to be within the image preview bounds
            startX = Math.max(0, Math.min(startX, rect.width));
            startY = Math.max(0, Math.min(startY, rect.height));

            console.log(`[Cropping-START] Mouse client: (X: ${e.clientX}, Y: ${e.clientY})`);
            console.log(`[Cropping-START] Relative to image: (StartX: ${startX}, StartY: ${startY})`);
            ...
            performCropBtn.classList.add('hidden');
            performCropBtn.disabled = true; // Disable until a valid selection is made
        }

        function drawRect(e) {
            if (!isDrawing) return;

            // Get bounding rect of the original image *preview*
            const rect = originalImagePreview.getBoundingClientRect();
            const currentX = e.clientX - rect.left;
            const currentY = e.clientY - rect.top;

            // Clamp currentX, currentY to be within the image preview bounds
            ...
            // Calculate top-left corner of the selection rectangle for display
            const displayLeft = Math.min(startX, clampedCurrentX);
            const displayTop = Math.min(startY, clampedCurrentY);

            cropSelectionRect.style.left = `${displayLeft}px`;
            cropSelectionRect.style.top = `${displayTop}px`;
            cropSelectionRect.style.width = `${width}px`;
            cropSelectionRect.style.height = `${height}px`;

            console.log(`[Cropping-DRAW] Mouse client: (CurrentX: ${e.clientX}, CurrentY: ${e.clientY})`);
            console.log(`[Cropping-DRAW] Clamped relative: (ClampedX: ${clampedCurrentX}, ClampedY: ${clampedCurrentY})`);
            console.log(`[Cropping-DRAW] Display Rect: L:${displayLeft.toFixed(2)}, T:${displayTop.toFixed(2)}, W:${width.toFixed(2)}, H:${height.toFixed(2)}`);

            // Calculate scaling factors from displayed size to natural (original) size
            const displayedWidth = originalImagePreview.clientWidth;
            const displayedHeight = originalImagePreview.clientHeight;

            console.log(`[Cropping-DRAW] Image Displayed: W:${displayedWidth}, H:${displayedHeight}`);
            console.log(`[Cropping-DRAW] Image Natural: W:${imageNaturalWidth}, H:${imageNaturalHeight}`);

            // Avoid division by zero if image not loaded or has no dimensions
            if (imageNaturalWidth === 0 || imageNaturalHeight === 0 || displayedWidth === 0 || displayedHeight === 0) {
                 console.warn('[Cropping] Image natural/displayed dimensions are zero, cannot calculate cropRect. Ensure image is fully loaded.');
            ...
            console.log(`[Cropping-DRAW] Scale Factors: X:${imgScaleX.toFixed(4)}, Y:${imgScaleY.toFixed(4)}`);


            // Calculate crop coordinates in natural image pixels
            let newCropX = Math.floor(displayLeft * imgScaleX);
            let newCropY = Math.floor(displayTop * imgScaleY);
            let newCropWidth = Math.floor(width * imgScaleX);
            let newCropHeight = Math.floor(height * imgScaleY);
            
            // Clamp cropRect coordinates to stay within natural image bounds
            newCropX = Math.max(0, Math.min(newCropX, imageNaturalWidth));
            newCropY = Math.max(0, Math.min(newCropY, imageNaturalHeight));
            newCropWidth = Math.max(0, Math.min(newCropWidth, imageNaturalWidth - newCropX));
            newCropHeight = Math.max(0, Math.min(newCropHeight, imageNaturalHeight - newCropY));

            cropRect.x = newCropX;
            cropRect.y = newCropY;
            cropRect.width = newCropWidth;
            cropRect.height = newCropHeight;

            console.log(`[Cropping-DRAW] Calculated CropRect (Natural Pixels): X:${cropRect.x}, Y:${cropRect.y}, W:${cropRect.width}, H:${cropRect.height}`);
        ...
        function stopDrawing() {
            console.log('[Cropping] stopDrawing called. Final cropRect:', cropRect);
            isDrawing = false;
            // Only hide the rect and disable button if the selection is invalid
            if (cropRect.width === 0 || cropRect.height === 0) {
                cropSelectionRect.classList.add('hidden'); // Hide if no valid selection
                performCropBtn.disabled = true;
                performCropBtn.classList.add('hidden');
                showToast('No valid crop area selected. Please drag to draw a rectangle.', 'warning');
            }
            // Keep overlay active for further selections until another filter is chosen or "Crop Image" is clicked
        }
        ...
        performCropBtn.addEventListener('click', () => {
            console.log('[Cropping] Perform Crop button clicked. Current cropRect:', cropRect);
            if (cropRect.width > 0 && cropRect.height > 0) {
                triggerImageProcessing();
                deactivateCropDrawing(); // Deactivate drawing after crop is performed
                cropSelectionRect.classList.add('hidden'); // Hide selection rect
                performCropBtn.classList.add('hidden'); // Hide button after cropping
            } else {
                showToast('Please select a valid cropping area first.', 'warning');
            }
        });
        ...
        async function processImage(filterName, params = {}) {
            ...
            if (filterName === 'crop') {
                ...
                console.log('[Process Image] Sending crop_coords payload:', payload.crop_coords);
            }
        ...
        imageUpload.addEventListener('change', function(event) {
            ...
            console.log('[Image Upload] File selected:', file); 

            if (file) {
                const reader = new FileReader();
                reader.onload = function(e) {
                    if (!e.target.result) {
                        console.error('[Image Upload] FileReader result is empty or null.');
                        showToast('Failed to read image file data.', 'error');
                        return;
                    }
                    currentImageBase64 = e.target.result.split(',')[1];
                    console.log('[Image Upload] Data URL prefix:', e.target.result.substring(0, e.target.result.indexOf(','))); 
                    console.log('[Image Upload] Data URL length:', e.target.result.length); 
                    console.log('[Image Upload] Base64 part length:', currentImageBase64.length); 
                    ...
                    try {
                        ...
                        console.log('[Image Display] Original image src assignment attempted.'); 
                    } catch (err) {
                        console.error('[Image Display] Synchronous error during src assignment:', err);
                        showToast('Error during image display initialization.', 'error');
                        return; 
                    }
                    ...
                    originalImagePreview.onload = () => {
                        ...
                        console.log(`[Image Display] Original Image Loaded: Natural Dimensions: ${imageNaturalWidth}x${imageNaturalHeight}, Displayed Dimensions: ${originalImagePreview.clientWidth}x${originalImagePreview.clientHeight}`);
                        showToast('Image displayed successfully in original preview.', 'success'); 
                    };
                    originalImagePreview.onerror = (error) => {
                        console.error('[Image Display] Error loading image into preview (async):', error);
                        showToast('Error displaying original image. It might be corrupt or an unsupported format.', 'error');
                    };
                ...
                reader.onerror = function() {
                    console.error('[FileReader] Error reading file from input:', reader.error); 
                    showToast('Error reading file from your device!', 'error');
                };
                reader.readAsDataURL(file);
            } else {
        ...
        function triggerImageProcessing() {
            ...
            } else if (selectedFilter === 'selective_color') {
                params.selective_color_target = document.getElementById('selectiveColorTarget').value;
                params.selective_hue_adjust = parseInt(document.getElementById('selectiveHueAdjustSlider').value);
                params.selective_saturation_adjust = parseInt(document.getElementById('selectiveSaturationAdjustSlider').value);
                params.selective_lightness_adjust = parseInt(document.getElementById('selectiveLightnessAdjustSlider').value);
            } else if (selectedFilter === 'color_splash') {
                params.color_splash_target_color = document.getElementById('colorSplashTargetColor').value;
                params.color_splash_tolerance = parseInt(document.getElementById('colorSplashToleranceSlider').value);
...
def process_image_route():
    ...
    with image_data_lock:
        ...
        params = {k: v for k, v in data.items() if k not in ['image_data', 'filter']}
        ...
        try:
            ...
            # Update the global original image if this is a new upload or reset
            # For filters, we always work on the 'current_original_img_np'
            if filter_name == "original":
                current_original_img_np = img_np.copy()
            elif current_original_img_np is None:
                 # If a filter is applied but original is not set (e.g., direct filter apply),
                 # set the uploaded image as original.
                 current_original_img_np = img_np.copy()

            # Ensure image_np passed to apply_filter is the original for most operations,
            # unless the filter itself is a transformation that should be chained.
            # For 'crop', it should apply to the original image's dimensions.
            if filter_name == 'crop':
                # For cropping, we need the actual original image's dimensions for coordinate calculation
                processed_img_np = apply_filter(current_original_img_np.copy(), filter_name, **params)
            else:
                # For all other filters, apply them to the current_original_img_np
                processed_img_np = apply_filter(current_original_img_np.copy(), filter_name, **params)


            is_success, buffer = cv2.imencode(".jpg", processed_img_np)
            if not is_success:
                return jsonify({'error': 'Could not encode image to JPG'}), 500

            processed_image_b64 = base64.b64encode(buffer).decode('utf-8')
            return jsonify({'processed_image': processed_image_b64})

        except Exception as e:
            print(f"Server-side error during image processing: {e}")
